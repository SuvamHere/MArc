...
function resizeCanvas() {
    ...
    const hintsH    = keyHints ? keyHints.getBoundingClientRect().height  : 0;
    const inputBarH = inputBar ? inputBar.getBoundingClientRect().height  : 0;

    canvas.width  = window.innerWidth;
    canvas.height = window.innerHeight - hudH - hintsH - inputBarH;
}
...
function generateEquation() {
    ...
    else if (lvl <= 4) pool = ['add_med', 'sub_med', 'mul_easy', 'add_med', 'sub_med', 'mul_med'];
    ...
    else               pool = ['mul_hard', 'div_med', 'add_hard', 'sub_hard', 'sq_med', 'cube', 'combo_add', 'combo_sub', 'percent'];
...
function on(id, event, fn) {
    ...
    if (el) {
        ...
        console.warn('NUMSTRIKE: element not found —', id);
    }
}

on('btn-start',        'click', startGame);
on('btn-restart',      'click', startGame);
on('btn-instructions', 'click', goToArcade);
on('btn-back-over',    'click', goToArcade);
on('btn-submit',       'click', submitAnswer);
